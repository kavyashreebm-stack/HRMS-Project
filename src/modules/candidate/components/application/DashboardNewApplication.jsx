...
import { applyForJob, getCandidateApplications } from "../../../../api";
...
export default function DashboardNewApplication({ currentUser, onView }) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      // 1. Prepare Backend Payload (Mapping frontend formData to backend schema)
      const payload = {
      ...
      // 3. Refresh List and Reset UI
      await fetchApps();
      setFormData({});
      setIsDirty(false);
      alert("Application submitted successfully and saved to database!");
      setView("list");
      ...
      {applications.length === 0 ? (
        <div className="flex justify-center items-center h-full">
          <div className="bg-white rounded-2xl shadow-lg p-10 max-w-md w-full text-center">
            ...
            <p className="text-gray-500 mb-6">
              You haven't submitted any job applications.
              Start your career journey by applying today.
            </p>
          </div>
        </div>
      ) : (
        <div className="space-y-4">
          {applications.map((app) => (
            <div
              key={app.id}
              className="shadow-sm border-gray p-4 rounded-lg flex justify-between items-center"
            >
              <div>
